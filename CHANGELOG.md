...
### v1.4.3 (2026-09-16)
**策略组**
- `✨ Siri AI` 候选末位补充 `🎯 全球直连`，为国行 Apple Intelligence 落地预留。
  当前默认仍为日本节点，不影响现有行为。适用场景：国行版沿用现有域名但解析至
  境内时，走代理反而错误。若国行版启用全新域名则需另补规则

### v1.4.2 (2026-09-16)
**策略组**
- `✨ 苹果智能` 更名 `✨ Siri AI`：与 iOS 27 的官方功能名保持一致
  （设置 → Siri → Try Siri AI）。仅组名变更，候选与规则指向不变

### v1.4.1 (2026-09-16)
**策略组**
- `🍏 苹果智能` 更名 `✨ 苹果智能`：Unicode 仅有 🍎 / 🍏 两个苹果，二者形状相同、
  色相接近，在策略组列表中小尺寸下难以区分。✨ 贴合 Apple Intelligence 自身的
  发光视觉标识，且与 `🍎 苹果服务` 一眼可辨。仅组名变更，候选与规则指向不变
